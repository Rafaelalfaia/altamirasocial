...
<div class="max-w-xl mx-auto bg-white p-6 rounded shadow">
    ...
    <form action="{{ route('restaurante.coordenador.cidadaos.store') }}" method="POST" onsubmit="return validarCPF()">
        @csrf

        <div class="mb-4">
            <label for="nome" class="block text-sm font-medium text-gray-700">Nome Completo</label>
            <input type="text" name="nome" id="nome" value="{{ old('nome') }}" class="w-full border rounded px-3 py-2 mt-1" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="email" name="email" id="email" value="{{ old('email') }}" class="w-full border rounded px-3 py-2 mt-1">
        </div>
