        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-gray-50 p-4 rounded border">
                ...
                <form method="POST" action="{{ route('coordenador.moradia.cidade.salvar') }}" class="space-y-2 mb-4">
                    ...
                    <button type="submit" class="w-full bg-blue-600 text-white px-3 py-2 rounded hover:bg-blue-700">Salvar
                        Cidade</button>
                </form>
                <ul class="text-sm space-y-1 max-h-48 overflow-y-auto">
                    @foreach($cidades as $cidade)
                        <li class="flex justify-between items-center bg-white px-2 py-1 border rounded">
                            <span>{{ $cidade->nome }} <span
                                    class="text-gray-400">({{ $cidade->estado->nome ?? '-' }})</span></span>
            ...
            <div class="bg-gray-50 p-4 rounded border">
                ...
                <form method="POST" action="{{ route('coordenador.moradia.bairro.salvar') }}" class="space-y-2 mb-4">
                    ...
                    <button type="submit" class="w-full bg-green-600 text-white px-3 py-2 rounded hover:bg-green-700">Salvar
                        Bairro</button>
                </form>
                        ...
                        <li class="flex justify-between items-center bg-white px-2 py-1 border rounded">
                            <span>{{ $bairro->nome }}
                                <span class="text-gray-400">
                                    ({{ $bairro->cidade->nome ?? '-' }}, {{ $bairro->cidade->estado->nome ?? '-' }})
                                </span>
                            </span>
