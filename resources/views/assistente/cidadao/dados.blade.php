...
@section('title', 'Editar Dados do Cidadão')
...
@php
  ...
  // Datas no padrão do cidadão
  $nasc = old('data_nascimento');
  if ($nasc === null) {
      $raw = $cidadao?->data_nascimento;
      $nasc = $raw
        ? ($raw instanceof \Carbon\CarbonInterface ? $raw->format('Y-m-d') : Carbon::parse($raw)->format('Y-m-d'))
        : '';
  }
  ...
  // CPF mascarado (visual) / banco sem máscara (feito no controller)
  ...
  // Opções padrão (iguais no cidadão)
...
<div class="max-w-5xl mx-auto">
  @includeWhen(true, 'assistente.cidadao.partials._steps', ['cidadao'=>$cidadao, 'atual'=>'dados'])
  ...
  <form method="POST"
        action="{{ route('assistente.cidadao.dados.salvar', $cidadao->id) }}"
    ...
    {{-- Cabeçalho com foto (padrão do cidadão) --}}
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="min-w-0">
          <div class="flex items-center gap-2">
            <h1 class="text-white font-semibold text-lg sm:text-xl truncate">{{ $cidadao->nome }}</h1>
            <span class="hidden sm:inline text-white/80 text-xs">· ID #{{ $cidadao->id }}</span>
          </div>
          <p class="sm:hidden text-white/80 text-xs mt-0.5">ID #{{ $cidadao->id }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="space-y-3">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div class="sm:col-span-3">
          <label class="text-xs font-medium text-gray-700">Nome <span class="text-red-500">*</span></label>
          <input type="text" name="nome" autocomplete="name"
                 value="{{ old('nome', $cidadao->nome) }}"
                 class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400">
        ...
        <div>
          ...
          <input type="date" name="data_nascimento" value="{{ $nasc }}"
                 class="w-full border rounded-lg px-3 py-2 focus:ring-indigo-200">
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
          ...
          <input type="tel" name="telefone" autocomplete="tel"
                 value="{{ old('telefone', $cidadao->telefone) }}"
                 class="w-full border rounded-lg px-3 py-2 focus:ring-indigo-200">
        </div>

        <div>
          <label class="text-xs font-medium text-gray-700">WhatsApp</label>
          <input type="tel" name="whatsapp" value="{{ old('whatsapp', $cidadao->whatsapp) }}"
                 class="w-full border rounded-lg px-3 py-2 focus:ring-indigo-200">
        </div>
    ...
    {{-- Seleções no padrão do cidadão (chips) --}}
    <div class="space-y-6">
      {{-- Sexo --}}
      <div class="space-y-2" x-data>
        <label class="text-xs font-medium text-gray-700">Sexo</label>
        {{-- input real que vai para o backend --}}
    ...
    {{-- Barra de ações fixa no mobile (mesmo padrão) --}}
    <div class="sticky bottom-0 left-0 right-0 -mx-4 sm:mx-0 bg-white/90 backdrop-blur border-t border-gray-100 px-4 sm:px-0 py-3">
      <div class="flex items-center justify-between">
        <a href="{{ route('assistente.usuarios.index') }}" class="text-sm text-gray-600 hover:text-gray-800">← Voltar à lista</a>
        <div class="flex items-center gap-2">
...
{{-- Alpine helpers: CPF + Foto + Chips (sexo/civil/raça) --}}
<script>
