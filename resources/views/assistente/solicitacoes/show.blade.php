...
<div class="max-w-4xl mx-auto bg-white p-6 rounded-xl shadow space-y-6">
    ...
    @if($solicitacao->status === 'pendente')
        <form method="POST" action="{{ route(request()->routeIs('cidadao.*') ? 'cidadao.solicitacoes.responder' : 'assistente.solicitacoes.responder', $solicitacao->id) }}" class="space-y-3">
            @csrf
    ...
    <div class="flex gap-2">
        @if($solicitacao->status === 'pendente')
            <form method="POST" action="{{ route(request()->routeIs('cidadao.*') ? 'cidadao.solicitacoes.concluir' : 'assistente.solicitacoes.concluir', $solicitacao->id) }}">
                @csrf
                @method('POST')
        ...
        @if($solicitacao->status === 'concluida')
            <form method="POST" action="{{ route(request()->routeIs('cidadao.*') ? 'cidadao.solicitacoes.destroy' : 'assistente.solicitacoes.destroy', $solicitacao->id) }}"
