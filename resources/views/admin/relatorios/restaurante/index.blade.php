@extends('layouts.app')
@section('title','Relatórios — Restaurante Popular')
...
<div class="max-w-7xl mx-auto px-6 py-8" x-data="ChartPage('12m')" x-init="boot()">
  ...
  <div class="flex flex-wrap gap-2 mb-6">
    ...
    <a href="{{ route('admin.relatorios.restaurante.index') }}" class="px-3 py-2 rounded-xl text-white bg-green-700">Restaurante</a>
  </div>

  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <h1 class="text-2xl md:text-3xl font-bold text-green-900">Restaurante popular</h1>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
    <div class="bg-white rounded-2xl shadow p-4">
      <h3 class="font-semibold text-gray-800 mb-2">Vendas (período)</h3>
      <div class="h-72">
        <canvas id="restVendas" data-type="line"
                data-route="{{ route('admin.relatorios.restaurante.vendas') }}"></canvas>
      </div>
    </div>

    <div class="bg-white rounded-2xl shadow p-4">
      <h3 class="font-semibold text-gray-800 mb-2">Consumo por Restaurante</h3>
      <div class="h-72">
        <canvas id="restConsumo" data-type="bar"
                data-route="{{ route('admin.relatorios.restaurante.consumo') }}"></canvas>
      </div>
    </div>

    <div class="bg-white rounded-2xl shadow p-4">
      <h3 class="font-semibold text-gray-800 mb-2">Formas de Pagamento</h3>
      <div class="h-72">
        <canvas id="restPagamento" data-type="doughnut"
                data-route="{{ route('admin.relatorios.restaurante.pagamento') }}"></canvas>
      </div>
    </div>
  </div>
</div>
